<template>
  ...
  <div class="heroSuite columnAlignCenter">
    ...
    <p class="text-lila my-3">
      If you wish to modify or remove the payment method, you need to add a new
      one first and then delete the one that will no longer be used.
    </p>
    <v-skeleton-loader
      ...
      >
        <div v-for="card in cards" :key="card.id" class="cardContainer">
          <div v-if="card.id === defaultCard" class="d-flex ml-5">
            <p class="w-auto bg-suiteGray rounded-t-lg elevation-3 py-1 px-2">
              Default
            </p>
          </div>
