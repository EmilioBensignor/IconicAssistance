<template>
	...
	<div class="columnAlignCenter mt-5">
		...
		<p class="w-75 text-red text-center mt-5" v-if="errorMsg">{{errorMsg}}</p>
		<SecondaryBtnComponent id="submit" @click.prevent="getSetupIntent()" v-if="!loading" text="Add Payment Method" :loading="loadingAttach"></SecondaryBtnComponent>
	</div>
</template>
...
onMounted(async () => {
	...
	const createSetupIntent = httpsCallableFromURL(
		...
		const res = await createSetupIntent({ userId: store.user.uid }).catch((error)=>{console.log(error)})
		clientSecret.value = res.data.clientSecret;
		const elements = stripe.elements( { clientSecret: clientSecret.value, currency: 'usd'});
		cardElement.value = elements.create('card', {clientSecret: clientSecret.value, style});
...
async function getSetupIntent(){
	...
	const { setupIntent, error } = await stripe.confirmCardSetup(
		...
		if (error) {
			...
			await attatchPaymentMethod({ setup_intent: setupIntent.id, userId: store.user.uid })
				.then((res) => {
					console.log(res);
					loadingAttach.value = false;
