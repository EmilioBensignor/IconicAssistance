	...
	<div class="bg-white rounded-xl elevation-3 pa-5 taskTable">
		<div class="column">
			...
			<div>
				...
				<ToDoComponent v-else :task="task" v-for="(task, index) in todo" :key="index" :moveTask="completeTask"
					:deleteTask="deleteTask" :openModal="openModalUpdate" />
			</div>
			<div>
				<div class="titles mt-3 pb-3">
					<p>Completed</p>
				...
				<ToDoComponent v-else :task="task" v-for="(task, index) in completed" :key="index" :moveTask="todoTask"
					:deleteTask="deleteTask" :openModal="openModalUpdate" />
			</div>
		</div>
		<v-dialog v-model="openDialog">
			<v-card class="bg-suiteBg align-self-center rounded-lg" max-width="100%"
				:title="modalMode === 'update' ? 'Update task' : 'New task'">
				<form class="column ga-3 mt-3 px-5">
					<div class="column ga-3">
						<label for="name">Task</label>
						<v-text-field id="name" required v-model="newTask.name"></v-text-field>
						<p v-if="nameError !== null" class="text-red">{{nameError}}</p>
					</div>
					<div class="column ga-3">
						<label for="due_date">Due date</label>
						<v-date-picker id="due_date" :min="yesterday" required width="100%"
							v-model="newTask.due_date"></v-date-picker>
							<p v-if="dueDateError !== null" class="text-red">{{dueDateError}}</p>
					</div>
					<div class="column ga-3">
						<label for="observations">Observations</label>
						<v-text-field id="observations" v-model="newTask.observations"></v-text-field>
					</div>
					<SecondaryBtn v-if="modalMode !== 'update'" class="align-self-end mb-5" text="Create"
						@click.prevent="createNewTask" :loading="loading">
					</SecondaryBtn>
					<SecondaryBtn v-else class="align-self-end mb-5" text="Update" @click.prevent="updateTask"
						:loading="loading">
					</SecondaryBtn>
				</form>
			</v-card>
	...
	methods: {
		...
		async deleteTask(task) {
			...
			if (todoIndex !== -1) {
				...
				this.completed.splice(this.completed.findIndex(t => t.name === task.name), 1)
			}
			const ref = doc(db, "assistants", this.assistantId);
			await updateDoc(ref, { tasks: this.tasks })
				.then(() => {
		...
		async updateTask() {
			...
			const index = this.tasks.findIndex(t => t.name === updatedTask.name)
			this.tasks[index] = updatedTask
			const todoIndex = this.todo.findIndex(t => t.name === updatedTask.name)
			if (todoIndex !== -1) {
				this.todo[todoIndex] = updatedTask
			} else {
				this.completed[this.completed.findIndex(t => t.name === task.name)] = updatedTask
			}
			const ref = doc(db, "assistants", this.assistantId);
			await updateDoc(ref, { tasks: this.tasks })
				.then(() => {
