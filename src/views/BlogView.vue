<template>
  ...
  <section class="skyRadioactive">
    ...
    <v-form class="w-75 buscador my-5 rounded-xl">
      <input type="search" name="blogSearch" v-model="blogSearch"
        class="inputSearch w-100 bg-white rounded-xl elevation-5 py-3 px-5" placeholder="What do you want to learn?"
        hide-details />
    </v-form>
    <div class="w-75 blogPosts columnAlignCenter ga-13 mt-5">
      <article v-for="(item, index) in filteredBlogs" :key="index" v-motion="scrollBottom"
        class="w-75 columnAlignCenter bg-white rounded-lg elevation-7">
        <router-link :to="`/blog-post/${item.slug}`">
          <img :src="getImgUrl(item.img)" :alt="item.alt" class="rounded-t-lg" width="100%" eager />
        </router-link>
        <div class="column ga-4 pa-5 pt-3">
          ...
          <router-link class="secondaryButton elevation-5 mt-2" :to="`/blog-post/${item.slug}`">Read Full
            Post</router-link>
        </div>
      </article>
    </div>
